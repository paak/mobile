...
//------------------------------
if ($AID==""){
	 echo "Can't save data because invalid  AID." ;
...
//------Find device token--------------------------					
	...
	/*$SQL_device="select distinct(devicetoken) as devicetoken,app_device,attendeeId from conferencedb.app_device_noti where  network='".$NETWORK."'  and conferenceID=".$id."  and (devicetoken is not null and  devicetoken <>'' and  devicetoken <>'(null)')  and app_device ='".$app_device."'  and attendeeId=23   ";*/
...
$SQL_device="select distinct(devicetoken) as devicetoken,app_device,attendeeId from conferencedb.app_device_noti where network='SINO' and conferenceID=83 and (devicetoken is not null and devicetoken <>'' and devicetoken <>'(null)') and app_device ='".$app_device."' and attendeeId not in ( select device_memid from conferencedb.history_noti where section ='ann' and conferenceid=83 and announcementid =".$AID." and app_device='".$app_device."' )";
	...
	for($x = 0; $x < $arrlength; $x++) {
		...
		if ($attendeeid != "" && $attendeeid !="(null)"){
				...
				/*$SQL_noti = "Insert into conferencedb.history_noti(timeStamp,userID,section,MSG,IP,updateBy,device_memid,AnnouncementID,network,conferenceID) values(NOW(),".$attendeeid.",'ann','".protect_quote($message)."','".$_SERVER['REMOTE_ADDR']."','admin',".$attendeeid.",".$AnnouncementID.",'".$NETWORK."',".$EventID.")";*/

				$SQL_noti = "Insert into conferencedb.history_noti(timeStamp,userID,section,MSG,IP,updateBy,device_memid,AnnouncementID,network,conferenceID,app_device) values(NOW(),".$attendeeid.",'ann','".protect_quote($message)."','".$_SERVER['REMOTE_ADDR']."','admin',".$attendeeid.",".$AnnouncementID.",'".$NETWORK."',".$EventID.",'ANDROID')";
	...
	for($x = 0; $x < $arrlength; $x++) {
		...
		if ($deviceToken != "" && $deviceToken !="(null)" && $attendeeId !=""){
			...
			echo "Send notification to Attendee ID : ".$attendeeId." has been completed.<br>";
